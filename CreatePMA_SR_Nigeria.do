...
#delimit;
use "$data/rawSDP/SDP_NGR2_Lagos.dta", clear ; 	keep if state==2 ; 	save "$data/rawSDP/SDP_NGLagosR2.dta", replace ; 
use "$data/rawSDP/SDP_NGR3_National.dta", clear ;keep if state==2 ; save "$data/rawSDP/SDP_NGLagosR3.dta", replace ; 
...
/*
foreach survey in NGLagosR2 NGLagosR3 NGLagosR4 NGLagosR5 NGKanoR3  NGKanoR4  NGKanoR5{
use "$data/rawSDP/SDP_`survey'.dta", clear
...
use "$data/rawSDP/SDP_`survey'.dta", clear	
	...
	* gen ClusterServed
	foreach num of numlist 1/25{
		capture confirm variable ClusterServed`num'
		if !_rc {
		*
		}
			else {
			tab state round 
			gen ClusterServed`num'=.
			}	
	}
	
	* gen EAserved
	foreach num of numlist 1/25{
		capture confirm variable EAserved`num'
		if !_rc {
		*
		}
			else {
			tab state round 
			rename ClusterServed`num' EAserved`num'
			}	
	}
...
*	pay extra attention to PILLS & INJECTABLES  
	...
	use "$data/rawSDP/SDP_NGLagosR2.dta", clear		
	foreach survey in $datalistminusone{
		...
		*rounds when JUST "pills" was asked*/ => ALL ROUNDS
...
*********************************************************	
	...
	* 		DEAL with different injectable names across surveys 
	...
	foreach survey in $datalist2A{
		...
		#delimit; 
		sum round provided_injectables stock_injectables stockout_3mo_injectables 
				  provided_sayana_press stock_sayana_press stockout_3mo_sayana_press
				  stockout_3mo_depo_provera provided_depo_provera stock_depo_provera 
				  ; 
				  #delimit cr
