...
global datalistminusone " 
	CDKinshasaR4 CDKinshasaR5 CDKinshasaR6 CDKinshasaR7 CDKinshasaP1
	CDKongoCentralR4 CDKongoCentralR5 CDKongoCentralR6 CDKongoCentralR7 CDKongoCentralP1
	";
	#delimit cr	
...
*********************************************************		
/*
cd "C:\Users\YoonJoung Choi\Dropbox\0 Data\PMA\rawSDP\"
dir 
*/

#delimit;
use "$data/rawSDP/SDP_CDR2_Kinshasa.dta", clear ; 		save "$data/rawSDP/SDP_CDKinshasaR2.dta", replace ;
use "$data/rawSDP/SDP_CDR3_Kinshasa.dta", clear ; 		save "$data/rawSDP/SDP_CDKinshasaR3.dta", replace ;
use "$data/rawSDP/SDP_CDR4_Kinshasa.dta", clear ; 		save "$data/rawSDP/SDP_CDKinshasaR4.dta", replace ;
use "$data/rawSDP/SDP_CDR5_Kinshasa.dta", clear ; 		save "$data/rawSDP/SDP_CDKinshasaR5.dta", replace ;
use "$data/rawSDP/SDP_CDR6_Kinshasa.dta", clear ; 		save "$data/rawSDP/SDP_CDKinshasaR6.dta", replace ;
use "$data/rawSDP/SDP_CDR7_Kinshasa.dta", clear ; 		save "$data/rawSDP/SDP_CDKinshasaR7.dta", replace ;

use "$data/rawSDP/SDP_CDR4_KongoCentral.dta", clear ; 	save "$data/rawSDP/SDP_CDKongoCentralR4.dta", replace ;
use "$data/rawSDP/SDP_CDR5_KongoCentral.dta", clear ; 	save "$data/rawSDP/SDP_CDKongoCentralR5.dta", replace ;
use "$data/rawSDP/SDP_CDR6_KongoCentral.dta", clear ; 	save "$data/rawSDP/SDP_CDKongoCentralR6.dta", replace ;
use "$data/rawSDP/SDP_CDR7_KongoCentral.dta", clear ; 	save "$data/rawSDP/SDP_CDKongoCentralR7.dta", replace ;

use "$data/rawSDP/SDP_CDP1.dta", clear ; keep if province==1; save "$data/rawSDP/SDP_CDKinshasaP1.dta", replace ;
use "$data/rawSDP/SDP_CDP1.dta", clear ; keep if province==2; save "$data/rawSDP/SDP_CDKongoCentralP1.dta", replace ;
#delimit cr	
...
* B. gen "round" 
*******************************************
foreach survey in CDKinshasaP1 CDKongoCentralP1 {
	use "$data/rawSDP/SDP_`survey'.dta", clear
...
*	pay extra attention to PILLS & INJECTABLES  
	...
	use "$data/rawSDP/SDP_CDKinshasaR3.dta", clear		
	foreach survey in $datalistminusone{
		...
		*rounds when JUST "pills" was asked*/ =>round 3-8
		tab round stock_pills, m 
	
		*DATALIST1: rounds when JUST "injectables" was asked*/ => NONE since round 3
		*tab round stock_injectables, m 
		
		*DATALIST2: rounds when "sayana_press vs. depo_provera" was asked*/ => round 3-7
...
*	For now, only India, Rahasthan 
	...
	*Three sets of datalist based on injectables variables
	global datalist1 ""  
	global datalist2 "CDKinshasaR3 CDKinshasaR4 CDKinshasaR5 CDKinshasaR6 CDKinshasaR7 CDKinshasaP1 CDKongoCentralR4 CDKongoCentralR5 CDKongoCentralR6 CDKongoCentralR7 CDKongoCentralP1"
