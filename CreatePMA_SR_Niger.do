...
global datalistminusone " 
	NENiameyR2 NENiameyR3 NENiameyR4 NENiameyR5
	NER2 NER4 
	";
	#delimit cr	
...
#delimit;
use "$data/rawSDP/SDP_NER1_Niamey.dta", clear 	; keep if region==1 ; save "$data/rawSDP/SDP_NENiameyR1.dta", replace ;
use "$data/rawSDP/SDP_NER2_National.dta", clear ; keep if region==1 ; save "$data/rawSDP/SDP_NENiameyR2.dta", replace ;
use "$data/rawSDP/SDP_NER3_Niamey.dta", clear 	; keep if region==1 ; save "$data/rawSDP/SDP_NENiameyR3.dta", replace ;
use "$data/rawSDP/SDP_NER4_National.dta", clear ; keep if region==1 ; save "$data/rawSDP/SDP_NENiameyR4.dta", replace ;
use "$data/rawSDP/SDP_NER5.dta", clear 			; keep if region==1 ; save "$data/rawSDP/SDP_NENiameyR5.dta", replace ;

use "$data/rawSDP/SDP_NER2_National.dta", clear	; save "$data/rawSDP/SDP_NER2.dta", replace ;
use "$data/rawSDP/SDP_NER4_National.dta", clear	; save "$data/rawSDP/SDP_NER4.dta", replace ;
#delimit cr	
...
*	pay extra attention to PILLS & INJECTABLES  
	...
	use "$data/rawSDP/SDP_NENiameyR1.dta", clear		
	foreach survey in $datalistminusone{
		...
		*rounds when JUST "pills" was asked*/ =>round 1-2
		tab round stock_pills, m 
	
		*DATALIST1: rounds when JUST "injectables" was asked*/ =>round 1-3
		tab round stock_injectables, m 
		
		*DATALIST2: rounds when "sayana_press vs. depo_provera" was asked*/ => round 4-5 
...
*	For now, only India, Rahasthan 
	...
	*Last round where JUST "injectables" was asked
	local last_round_inj 3
	
	*Three sets of datalist based on injectables variables
	global datalist1 "NENiameyR1 NENiameyR2 NENiameyR3 NER2 "  
	global datalist2 "NENiameyR4 NENiameyR5 NER4"
